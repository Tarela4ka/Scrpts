    ...
    local library = loadstring(game:HttpGet("https://ic-scripts.ml/script?name=library"))()
    getgenv().api = loadstring(game:HttpGet("https://ic-scripts.ml/script?name=api"))()
    local bssapi = loadstring(game:HttpGet("https://ic-scripts.ml/script?name=bssapi"))()
    ...
    local Config = {
        WindowName = "ICocmoc v" .. temptable.version .. " Re-Remastered By ICrazych",
        Color = Color3.fromRGB(32, 137, 229),
    ...
    local premtab = Window:CreateTab("Crack?")
    ...
    guiElements["toggles"]["farmcoco"] = farmo:CreateToggle("Farm Shower", nil, function(State) kocmoc.toggles.farmcoco = State end)
    ...
    local webhooksection = premtab:CreateSection("Webhook")
    ...
    guiElements["vars"]["webhookurl"] = webhooksection:CreateTextBox("Webhook URL", "Discord webhook URL", false, function(Value)
        if Value and string.find(Value, "https://") then
            ...
            api.notify("ICocmoc " .. temptable.version, "Invalid URL!", 2)
        end
    end)
    guiElements["vars"]["webhooktimer"] = webhooksection:CreateSlider("Minutes Between Updates", 1, 60, 60, true, function(Value)
    ...
    guiElements["vars"]["discordid"] = webhooksection:CreateTextBox("Discord ID", "", false, function(Value)
        if tonumber(Value) then
            ...
            api.notify("ICocmoc " .. temptable.version, "Invalid ID!", 2)
        end
    end)
    ...
    raresettings:CreateButton("Copy Token List Link", function()
        api.notify("ICocmoc " .. temptable.version, "Copied link to clipboard!", 2)
    ...
    kocmocs:CreateButton("Load Config", function()
        if not isfile("kocmoc/BSS_" .. temptable.configname .. ".json") then
            api.notify("ICocmoc " .. temptable.version, "No such config file!", 2)
        else
    ...
    local density,friction,elasticity,frictionWeight,elasticityWeight = 5,0.1,1,1,1
    ...
    game.Workspace.Particles.ChildAdded:Connect(function(v)
        if (v:IsA("Part") or v:IsA("MeshPart")) and not temptable.started.ant and not temptable.started.vicious and kocmoc.toggles.autofarm and not temptable.converting and not temptable.planting then
            if v.Name == "WarningDisk" and v.Size.X == 8 and kocmoc.toggles.farmcoco then
                task.wait(0.5)
    ...
    task.spawn(function()
        print(os.time() - timestamp)
        print(kocmoc.vars.webhooktimer * 60)
        while task.wait(1) do
    ...
    if _G.autoload then
        ...
        if not set_thread_identity then
            api.notify("ICocmoc " .. temptable.version, "your exploit only partially supports autoload!", 2)
        else
